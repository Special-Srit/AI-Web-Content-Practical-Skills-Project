...
| B. Basecoat | shadcn v4를 **plain HTML + Tailwind**로 이식한 MIT 라이브러리. 클래스·테마 변수가 shadcn과 호환 | 그대로 사용 | 그대로 | 상호작용 컴포넌트만 |
| C. 직접 작성 CSS | 같은 변수명으로 손으로 작성. 의존성 0 | 그대로 사용 | 그대로 | 컴포넌트 전부 |

- **B가 기존 대비안(sol로 React→vanilla 변환)보다 우수** — 변환이 아니라 **같은 토큰을 쓰는 다른 구현**이므로 디자인·토큰이 1:1로 옮겨진다. React 금지 판정이 나와도 피그마 작업은 폐기되지 않음
- 단 B·C 모두 **Tailwind를 쓸지**가 별도 확인 사항 → §6
...
| `--destructive` | `oklch(0.577 0.245 27.325)` | 금지 목록 — **§3 톤 제약 확인 필요** |
...
- **`--destructive`를 금지 목록에 그대로 쓰면 안 될 수 있음** — 03 §11-6이 금지 목록 톤을 `훈계·죄책감 유발 금지`로 제한했다. 빨강 경고색은 그 제약과 충돌하므로, 색은 `muted` 계열로 두고 구분은 아이콘·라벨로 하는 편이 안전 [판단]
...
| Input | `h-9` | 36 | — | 미달 |
| Tabs trigger | `h-9` 안 | 36 | 하단 탭 48×48 | 미달 |
...
- 하단 탭 히트 영역 **48×48** — 피그마 파일은 현재 84×48로 세로만 충족
...
- shadcn 기본은 Button·Select·Tabs가 `text-sm` = **14px**, Input·Textarea만 `text-base` = **16px**
- 3-1 강사 규칙 — 본문 **최소 16px**, 작은 글씨 **최소 14px**, 스크린은 **고딕체만**(명조 금지)
- 판정 — **버튼·탭 라벨을 16px로 올린다.** 버튼 라벨은 `작은 글씨`가 아니라 본문급 조작 대상
- Input이 이미 `text-base`인 것은 우연이 아니라 **iOS Safari가 16px 미만 입력에 포커스하면 페이지를 확대**하는 문제를 피하기 위한 것 → 모바일 우선인 우리 프로젝트에서 **절대 낮추지 말 것**
- 강사가 지정할 **폰트 3종은 아직 미발표** → 피그마에서는 우선 자리 표시 서체로 두고, 발표 후 토큰만 교체

## 5. 화면별 컴포넌트 매핑 (04-design §6의 9화면)
...
| S2 선택지 5개 | RadioGroup 또는 Button `outline` | 단일 선택 + `다음` 확정형이므로 RadioGroup이 의미상 정확 |
...
| S3-c 저장 시트 | **Drawer** (하단) + Input + Button | Drawer는 shadcn에서 Beta 표기 |
...
- **shadcn/ui에는 모바일 하단 탭 바 컴포넌트가 없다.** 데스크톱 지향이라 Sidebar만 제공한다. 우리 IA의 핵심인 4탭 셸은 **커스텀 컴포넌트**이며, Tabs를 재활용하면 `h-9`(36px)에 묶여 48×48을 못 맞춘다 → 처음부터 직접 만들 것
...
- **키트를 쓰더라도 §3의 44/48 오버레이는 직접 적용해야 한다** — 키트는 shadcn 기본 치수를 그대로 재현하므로 데스크톱 밀도를 물려받는다
