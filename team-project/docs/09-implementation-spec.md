...
- UI 컴포넌트는 **shadcn/ui**를 쓴다. Tailwind 기반이고 코드를 프로젝트 안으로
  복사해 오는 방식이라 직접 고칠 수 있다
- **환경 세팅은 `10-dev-setup.md`를 따른다** — Node 버전, Vite 생성, Tailwind,
  경로 별칭, shadcn 초기화, 배포 설정까지 그 문서 하나로 끝난다
- **하단 4탭은 shadcn `tabs`가 아니다.** 패널 전환이 아니라 목적지 이동이므로
  `NavFooter.jsx`로 직접 만든다 (§6)
